...
에어비앤비 지도 버전, 인기순 기능(count) ← log데이터를 남기지 않나?

- ::after 이란?
- 클래스명은 _ 붙이고 그에대한 간략한 기능에 대해서는 _를 안하기(기능의 의미)
- script type를 적어주는 이유는?
- html 에서 svg 태크란?
- class명과 id명을 같이 사용하는 이유는 무엇이지?
공통점을 묶을려고 하는건가?
- 각각의 div는 옆이 아닌 아래로 채워간다.
- div의 자식 div는 옆으로 채워진다.
- label 태그
- br 태그
- input onfocus 속성이란?
- input required 속성
- **html - HTML 'hidden'과 'aria-hidden'속성의 차이점은 무엇입니까?**
- **HTML과 CSS를 활용해서 콘텐츠를 숨기는 방법은?**
- 화면 다크모드로 변경하는 방법은? 아래 사이트 처럼
}
...
{
좋은 프론트엔드 개발자란 무엇일까?
시간이 지나도 오랫동안 사용할 수 있는 **'지속 가능한 코드를 쓸 수 있는지'**, 

그리고 웹 어플리케이션의 **'성능을 향상시키기 위해 코드를 개선하고 점검할 수 있는지'** 를 파악해야하며, 

계속되는 코드 테스트와 TDD, 리팩토링, BDD 등 코드를 견고하게 바꿔나가기 위한 일련의 과정을 거쳐 지속 가능한 코드를 갖추는 법을 먼저 배워야겠다.

좋은 개발자란 무엇인가?
의사소통과 이해 능력, 공감, 코드 유지보수 및 성능 향상, 리더십, 데드라인, 계획 기획 등을 골고루 잘하는 사람일 것 같다.
개발자 말고도 PM이 있기도 하지만 아직 자신은 무엇이 맞는 것인지 몰라 닥치는대로 해볼 것이다. 무조건 풀스택@!
